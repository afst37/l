...
if __name__ == '__main__':
    ...
    #process0 = Process(xieku(que))
    #process0.start()
    for file in files:  # 遍历文件夹
        if not os.path.isdir(file):  # 判断是否是文件夹，不是文件夹才打开
            filename = path +'/temp' + "/" + file  # 打开文件
            res=pool.apply_async(duwenjian, (filename,))
            job_result.append(res)
    pool.close() #关闭进程池
    pool.join()

    time01 = time.perf_counter()
    print('程序读取数据时间：' + str(time01-time00))
    ...
    print('程序合并数据时间：' + str(time02-time00))

    #转换数据为csv格式的StringIO
    output = StringIO()
    get_result.to_csv(output, sep=',', index=False, header=False)
